...
public class AVLTreeMap<K, V> extends TreeMap<K, V> {
    ...
    protected Position<Entry<K, V>> tallerChild(Position<Entry<K, V>> p) {
        if(height(left(p)) > height(right(p))){
            return left(p); //If the left child is taller, choose the left
        }
        if(height(left(p)) < height(right(p))){
            return right(p); //If the right child is taller, choose the left
        }
        if(isRoot(p)){
            return left(p); //Choice doesn't matter here
        }
        if(p == left(parent(p))){
            return left(p);
        } //Return whichever child is aligned
        return right(p);
    }
